...
export default function ReviewForm() {
    ...
    async function onSubmit(data: ReviewFormData) {
        ...
        const result = await saveCapture(user.uid, data, photoDataUri, confidence);
        setIsSubmitting(false);

        if (result.success) {
            toast({ title: 'Success!', description: `${data.breedName} added to your collection.` });
            clearCaptureData();
                    ...
                    <CardContent className="space-y-6">
                        {photoDataUri && (
                            <div className="relative aspect-video w-full overflow-hidden rounded-lg border">
                                <Image src={photoDataUri} alt="Captured dog" layout="fill" objectFit="cover" />
                            </div>
                        )}
                        ...
                        <div className="flex items-center justify-between rounded-lg bg-muted p-3">
                            ...
                            <span className="font-bold text-lg text-primary">{((confidence || 0) * 100).toFixed(0)}%</span>
                        </div>
