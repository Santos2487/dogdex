...
import { Calendar, ChevronLeft, Heart, Loader2, Percent, Tag, FileText } from 'lucide-react';
    ...
    useEffect(() => {
        if (user && id) {
            const getEntry = async () => {
                ...
                if (docSnap.exists()) {
                    setEntry({ id: docSnap.id, ...docSnap.data() } as DogEntry);
                } else {
        ...
        <div className="container mx-auto max-w-2xl p-4">
            ...
            <Card className="overflow-hidden">
                ...
                <CardHeader>
                    ...
                    {entry.name && <p className="text-lg text-muted-foreground -mt-1">{entry.breedName}</p>}
                </CardHeader>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        />
                         <DetailItem 
                            icon={Tag} 
                            label="Rarity"
                            value={<Badge variant={entry.rarity === 'Rare' ? 'destructive' : entry.rarity === 'Uncommon' ? 'secondary' : 'default'}>{entry.rarity}</Badge>}
                        />
                         <DetailItem 
                            icon={Heart} 
                            label="Favorite"
